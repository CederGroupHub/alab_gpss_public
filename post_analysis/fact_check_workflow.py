...
OUTPUT_DIR = ROOT_DIR / "post_analysis" / "fact_check_results"
...
5. If you find supporting literature, mark is_verified as true with high confidence
6. If you cannot find supporting literature, mark is_verified as false

Respond ONLY with a valid JSON object:
{{
    "is_verified": true,
    "confidence": 0.9,
    "explanation": "Verified with literature sources",
...
def verify_external_claim(claim: ExtractedClaim, search_effort: str = SEARCH_EFFORT_LOW) -> VerificationResult:
    ...
    if result:
        ...
        if is_verified:
            evidence.append("✓ Verified with web search")
        else:
            evidence.append("✗ Not verified (insufficient literature support)")
        ...
        is_verified = any(word in content.lower() for word in ["verified", "confirmed", "supported"])
        is_not_verified = any(word in content.lower() for word in ["not verified", "unverified", "no evidence", "cannot verify"])

        return VerificationResult(
            claim=claim,
            is_verified=is_verified and not is_not_verified,
            confidence=0.5 if is_verified else 0.3,
...
def reverify_unverified_external_claims(
    ...
    that were marked as not verified, and re-searches them with higher search effort.
    ...
    print("=" * 60)
    print(f"RE-VERIFYING UNVERIFIED EXTERNAL CLAIMS (effort: {search_effort})")
    print("=" * 60)

    with open(results_file, 'r', encoding='utf-8') as f:
        results = json.load(f)

    # Find unverified external claims
    external_results = results.get("external_verification_results", [])
    unverified = [r for r in external_results if not r.get("is_verified", False) and r.get("confidence", 0.0) <= 0.7]

    print(f"\nFound {len(unverified)} unverified external claims out of {len(external_results)} total")

    if start_from_claim > 0:
        if start_from_claim >= len(unverified):
            print(f"start_from_claim ({start_from_claim}) is >= total unverified claims ({len(unverified)}). Nothing to process.")
            return results
        unverified = unverified[start_from_claim:]
        print(f"Starting from claim index {start_from_claim} ({len(unverified)} remaining)")

    if max_claims:
        unverified = unverified[:max_claims]
        print(f"Limited to {max_claims} claims for re-verification")

    if not unverified:
        print("No unverified claims to process.")
        return results

    # Re-verify each unverified claim
    updated_count = 0
    newly_verified = 0

    total_to_process = len(unverified)
    for i, result_dict in enumerate(unverified):
        ...
        print(f"\n[{i+1}/{total_to_process}] (index {global_index}) Re-verifying: {claim.claim_text[:80]}...")
        ...
        try:
            ...
            for j, orig_result in enumerate(external_results):
                orig_claim_id = get_claim_id(ExtractedClaim(**orig_result["claim"]))
                if orig_claim_id == claim_id:
                    external_results[j] = {
                        "claim": asdict(new_result.claim),
                    ...
                    updated_count += 1
                    if new_result.is_verified:
                        newly_verified += 1
                        print(f"  [OK] Now verified (confidence: {new_result.confidence:.2f})")
                    else:
                        print(f"  [X] Still not verified (confidence: {new_result.confidence:.2f})")
    ...
    verified_count = sum(1 for r in external_results if r.get("is_verified", False))
    results["summary"]["external_referenced"]["verified"] = verified_count
    results["summary"]["external_referenced"]["not_verified"] = len(external_results) - verified_count
    ...
    print(f"Total verified: {verified_count}/{len(external_results)}")
    ...
    results = {
        ...
        "summary": {
            ...
            "external_referenced": {
                ...
                "verified": sum(1 for r in external_results if r.is_verified),
                "not_verified": sum(1 for r in external_results if not r.is_verified),
